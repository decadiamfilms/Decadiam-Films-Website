...
import { generateOrderTemplate } from '../orders/OrderTemplate';

interface FinalOrderModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function FinalOrderModal({
  isOpen,
  onClose,
  onOrderGenerated,
  orderData
}: FinalOrderModalProps) {
  ...
  const [emailSubject, setEmailSubject] = useState(`Order ${orderData.orderId} - ${orderData.projectName}`);
  const [emailBody, setEmailBody] = useState(`Dear ${orderData.customer.name},\n\nPlease find attached your order for ${orderData.projectName}.\n\nOrder Details:\n- Order ID: ${orderData.orderId}\n- Total Amount: $${orderData.totals.total.toFixed(2)} (inc GST)\n\nPlease don't hesitate to contact us if you have any questions.\n\nBest regards,\nYour SalesKik Team`);
  ...
  const loadTemplateAndProfile = async () => {
    try {
      ...
      if (savedSettings) {
        ...
        setPdfSettings(settings.documentSettings?.order || pdfSettings);
      }
  ...
  const generateOrder = () => {
    try {
      ...
      const htmlContent = generateOrderTemplate(orderData, globalStyling, companyProfile, pdfSettings);
      
      console.log('Generated HTML content length:', htmlContent.length);
      console.log('HTML preview:', htmlContent.substring(0, 200));
      
      if (!htmlContent || htmlContent.length < 100) {
        alert('Generated order content is empty or too short');
        return;
      }
      ...
      alert(`Error generating order: ${error}. Please try again.`);
    }
  };
  ...
  const downloadOrder = () => {
    try {
      ...
      link.href = url;
      link.download = `Order_${orderData.orderId}_${new Date().toISOString().split('T')[0]}.html`;
      ...
      alert('Error downloading order. Please try again.');
    }
  };
    ...
    <div className="fixed inset-0 bg-white bg-opacity-80 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl shadow-2xl max-w-6xl w-full max-h-[95vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Final Order</h2>
            <p className="text-gray-600">Configure and generate your professional order</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          {currentView === 'preview' && (
            <div>
              <p className="text-gray-600 mb-6">Your order is ready! Generate the final document using your template.</p>
          ...
          {currentView === 'settings' && (
            <div>
              ...
              <p className="text-gray-600 mb-6">Configure what information to include in the generated order.</p>
          ...
          {currentView === 'email' && (
            <div>
              ...
              <p className="text-gray-600 mb-6">Compose and send the order directly to your customer.</p>
