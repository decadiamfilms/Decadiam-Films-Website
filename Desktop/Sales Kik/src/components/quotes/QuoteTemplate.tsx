...
export const generateQuoteTemplate = (quoteData: any, globalStyling: any, companyProfile: any, pdfSettings: any) => {
    ...
    <div class="document-container">
        ...
        <div class="header">
            <div class="header-content">
                ...
                <div class="document-details">
                    ...
                    <div class="document-number">#${quoteData.quoteId}</div>
                    <div class="document-date">Date: ${new Date().toLocaleDateString()}</div>
                    ${quoteData.referenceNumber ? `<div class="reference-number">Ref: ${quoteData.referenceNumber}</div>` : ''}
                </div>
            </div>
        </div>
        ...
        <div class="main-content">
            ...
            <div class="section">
                ...
                <div class="two-column">
                    <div class="info-block">
                        ...
                        <p>${quoteData.customer.address.streetNumber} ${quoteData.customer.address.streetName}</p>
                        <p>${quoteData.customer.address.suburb}, ${quoteData.customer.address.state} ${quoteData.customer.address.postcode}</p>
                        ` : ''}
                        ${quoteData.customer.primaryContact?.email ? `<p>${quoteData.customer.primaryContact.email}</p>` : ''}
                        ${quoteData.customer.primaryContact?.mobile || quoteData.customer.phone ? `<p>${quoteData.customer.primaryContact.mobile || quoteData.customer.phone}</p>` : ''}
                    </div>
                    <div class="info-block">
                        <h3>Project Details</h3>
                        <p><strong>Project:</strong> ${quoteData.projectName || 'Quote Request'}</p>
                        ${quoteData.referenceNumber ? `<p><strong>Reference:</strong> ${quoteData.referenceNumber}</p>` : ''}
                        <p><strong>Valid Until:</strong> ${validUntilDate.toLocaleDateString()}</p>
                        <p><strong>Payment Terms:</strong> Net 30</p>
                        <p><strong>Delivery:</strong> ${quoteData.delivery?.method ? quoteData.delivery.method.charAt(0).toUpperCase() + quoteData.delivery.method.slice(1) : 'TBD'}</p>
                        ${quoteData.delivery?.contactName ? `<p><strong>Contact:</strong> ${quoteData.delivery.contactName}</p>` : ''}
                        ${quoteData.delivery?.contactPhone ? `<p><strong>Contact Phone:</strong> ${quoteData.delivery.contactPhone}</p>` : ''}
                    </div>
                </div>
            </div>
            ...
            <div class="section">
                ...
                ${quoteData.optionGroups && Object.keys(quoteData.optionGroups).length > 0 ? `
                ...
                <div class="totals">
                    <div class="totals-table">
                        <div class="totals-row">
                            ...
                            <span class="totals-value">$${quoteData.totals.subtotal.toFixed(2)}</span>
                        </div>
                        <div class="totals-row">
                            <span class="totals-label">GST (${((quoteData.totals.gst / quoteData.totals.subtotal) * 100).toFixed(1)}%):</span>
                            <span class="totals-value">$${quoteData.totals.gst.toFixed(2)}</span>
                        </div>
                        <div class="totals-row">
                            <span>Total:</span>
            ...
            <div class="section">
                ...
                <div class="two-column">
                    <div class="info-block">
                        ...
                        <p><strong>${(() => {
                            const method = quoteData.delivery.method;
                            switch(method) {
                        ...
                        ${quoteData.delivery.method === 'pickup' ? `
                        ...
                        <div style="margin-top: 8px; padding: 8px; background: #f3f4f6; border-radius: 4px;">
                            <p style="font-weight: 600; color: #374151;">${quoteData.delivery.method === 'courier' ? 'Courier Delivery Address:' : 'Delivery Address:'}</p>
                        ...
                        ${quoteData.delivery.isAddressConfirmed ? 
                    ...
                    <div class="info-block">
                        <h3>${quoteData.delivery.method === 'pickup' ? 'Pickup Coordinator' : 'Delivery Coordinator'}</h3>
                        ${quoteData.delivery.contactName ? `<p><strong>Primary Contact:</strong> ${quoteData.delivery.contactName}</p>` : ''}
                        ${quoteData.delivery.contactPhone ? `<p><strong>Contact Number:</strong> ${quoteData.delivery.contactPhone}</p>` : ''}
            ...
            ${quoteData.accounting && !pdfSettings.hidePaymentDetails ? `
            <div class="section">
                <h2 class="section-title">Accounting & Payment Details</h2>
                <div class="two-column">
                    <div class="info-block">
                        <h3>Chart of Accounts</h3>
                        <p><strong>Revenue Account:</strong> ${quoteData.accounting.accountNumber}</p>
                        <p><strong>Tax Classification:</strong> ${quoteData.accounting.gstRate.replace(/^(\d+)-(.+)/, (match, rate, description) => {
                            const rateText = rate + '% GST';
                            const descText = description.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
                            return `${rateText} - ${descText}`;
                        })}</p>
                        ${quoteData.totals.weight > 0 ? `<p><strong>Total Weight:</strong> ${quoteData.totals.weight.toFixed(1)} kg</p>` : ''}
                    </div>
                    <div class="info-block">
                        <h3>Financial Summary</h3>
                        <p><strong>Payment Terms:</strong> Net 30 days from invoice date</p>
                        <p><strong>GST Collected:</strong> $${quoteData.accounting.gstAmount.toFixed(2)}</p>
                        <p style="color: #059669; font-weight: 600;"><strong>Total Due:</strong> $${quoteData.totals.total.toFixed(2)}</p>
                    </div>
                </div>
            </div>
            ` : ''}
            ...
            ${quoteData.comments && !pdfSettings.hideCustomText ? `
            <div class="section">
                ...
                <div class="terms-content" style="background: #fef3c7; border: 1px solid #fbbf24; border-radius: 6px; padding: 16px;">
                    <div style="white-space: pre-wrap; color: #92400e; font-size: 13px; line-height: 1.6; font-weight: 500;">${quoteData.comments}</div>
            ...
            <!-- Terms & Conditions -->
            ${quoteData.standardText && !pdfSettings.hideCustomText ? `
            <div class="section">
                <h2 class="section-title">Terms & Conditions</h2>
                <div class="terms-content">
                    <div style="white-space: pre-wrap; color: #475569; font-size: 12px; line-height: 1.4;">${quoteData.standardText}</div>
                </div>
            </div>
            ` : ''}
        </div>
        ...
        <div class="section" style="margin-top: 30px; border-top: 1px solid #e5e7eb; padding-top: 20px;">
            <h2 class="section-title">Quote Acceptance</h2>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-top: 20px;">
                ...
                <div>
                    <p style="font-weight: 600; margin-bottom: 10px; color: #374151;">Authorized Representative</p>
                    <div style="border-bottom: 2px solid #d1d5db; height: 50px; margin-bottom: 10px;"></div>
                    <p style="font-size: 11px; color: #6b7280;">Print Name: _________________________</p>
                    <p style="font-size: 11px; color: #6b7280; margin-top: 5px;">Date: _________________________</p>
                </div>
            ...
            <div style="margin-top: 15px; padding: 12px; background-color: #f9fafb; border-radius: 6px; border-left: 4px solid ${globalStyling.primaryColor || '#3b82f6'};">
                <p style="font-size: 11px; color: #374151; line-height: 1.4;">
                    <strong>By signing above, you acknowledge that you have read and agree to the terms and conditions of this quote. 
                    This quote is valid for 30 days from the date issued.</strong>
                </p>
            </div>
        </div>
        ` : ''}
